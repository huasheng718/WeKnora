...
BEGIN
    IF NEW.id IS DISTINCT FROM OLD.id OR NEW.tenant_id IS DISTINCT FROM OLD.tenant_id OR
       NEW.project_id IS DISTINCT FROM OLD.project_id OR NEW.document_id IS DISTINCT FROM OLD.document_id OR
       NEW.version_id IS DISTINCT FROM OLD.version_id OR NEW.review_request_id IS DISTINCT FROM OLD.review_request_id OR
       NEW.release_digest IS DISTINCT FROM OLD.release_digest OR NEW.retention_days IS DISTINCT FROM OLD.retention_days OR
...
BEGIN
    ...
    END IF;
    IF NEW.status = 'active' AND NOT EXISTS (SELECT 1 FROM production_projection_heads WHERE tenant_id = NEW.tenant_id AND document_id = NEW.document_id AND target_knowledge_base_id = NEW.target_knowledge_base_id AND active_release_target_id = NEW.id) THEN
    ...
    END IF;
    IF OLD.status = 'active' AND NEW.status <> 'active' AND EXISTS (SELECT 1 FROM production_projection_heads WHERE tenant_id = OLD.tenant_id AND document_id = OLD.document_id AND target_knowledge_base_id = OLD.target_knowledge_base_id AND active_release_target_id = OLD.id) THEN
...
ALTER TABLE production_release_targets DROP CONSTRAINT chk_production_release_targets_failure;
ALTER TABLE production_release_targets DROP COLUMN failure_reason, DROP COLUMN failure_code;
ALTER TABLE production_releases DROP CONSTRAINT chk_production_releases_digest_version;
ALTER TABLE production_releases DROP COLUMN release_digest_version;
