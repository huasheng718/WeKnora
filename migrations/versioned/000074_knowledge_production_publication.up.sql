...
CREATE TABLE IF NOT EXISTS production_releases (
    ...
    CONSTRAINT fk_production_releases_supersedes_context FOREIGN KEY (supersedes_release_id, tenant_id, project_id, document_id, version_id)
...
BEGIN
    IF NEW.id IS DISTINCT FROM OLD.id OR
       ...
       NEW.supersedes_release_id IS DISTINCT FROM OLD.supersedes_release_id OR
...
ALTER TABLE production_release_targets
    ...
    ADD COLUMN recovery_attempted_at TIMESTAMP NULL;
...
BEGIN
    IF NEW.status <> 'building' OR NEW.failure_code <> '' OR NEW.failure_reason <> '' OR
       ...
       NEW.cleaned_at IS NOT NULL OR NEW.retention_until IS NOT NULL OR
       NEW.recovery_attempted_at IS NOT NULL THEN
        RAISE EXCEPTION 'production release targets must be created building';
    END IF;
    RETURN NEW;
END;
...
BEGIN
    IF NEW.id IS DISTINCT FROM OLD.id OR NEW.release_id IS DISTINCT FROM OLD.release_id OR
       ...
       NEW.retention_days IS DISTINCT FROM OLD.retention_days OR NEW.created_at IS DISTINCT FROM OLD.created_at THEN
        RAISE EXCEPTION 'production release target identity is immutable';
    END IF;
    IF OLD.status IN ('failed', 'rolled_back', 'cleanup_pending', 'cleaned') AND
       NEW.status = OLD.status AND
       NEW.retention_until IS DISTINCT FROM OLD.retention_until THEN
        RAISE EXCEPTION 'production release target retention_until is immutable after eligibility';
    END IF;
    IF ((OLD.status IN ('failed', 'rolled_back') AND NEW.status = 'cleanup_pending') OR
        (OLD.status = 'cleanup_pending' AND NEW.status = 'cleaned')) AND
       NEW.retention_until IS DISTINCT FROM OLD.retention_until THEN
        RAISE EXCEPTION 'production release target retention_until is immutable after eligibility';
    END IF;
    IF OLD.status IN ('cleanup_pending', 'cleaned') AND
       NEW.cleanup_requested_at IS DISTINCT FROM OLD.cleanup_requested_at THEN
        RAISE EXCEPTION 'production release target cleanup_requested_at is immutable after cleanup entry';
    END IF;
    IF OLD.status = 'cleaned' AND NEW.cleaned_at IS DISTINCT FROM OLD.cleaned_at THEN
        RAISE EXCEPTION 'production release target cleaned_at is immutable after completion';
    END IF;
    ...
    IF NEW.status IN ('building', 'ready') AND (NEW.activated_at IS NOT NULL OR NEW.failed_at IS NOT NULL OR NEW.rolled_back_at IS NOT NULL OR NEW.cleanup_requested_at IS NOT NULL OR NEW.cleaned_at IS NOT NULL OR NEW.retention_until IS NOT NULL) THEN
        RAISE EXCEPTION 'building and ready production release targets must not retain lifecycle timestamps';
    END IF;
    IF NEW.status = 'failed' AND (NEW.failure_code = '' OR NEW.failure_reason = '') THEN
        RAISE EXCEPTION 'failed production release targets require failure metadata';
    END IF;
...
CREATE OR REPLACE FUNCTION guard_production_release_target_recovery_attempt()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.recovery_attempted_at IS DISTINCT FROM OLD.recovery_attempted_at AND NOT (
       ...
       (OLD.status IN ('building', 'failed', 'rolled_back') AND NEW.status = 'building' AND
        ...
        RAISE EXCEPTION 'production release target recovery attempts are monotonic and building-only';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
CREATE TRIGGER trg_production_release_targets_guard_recovery_attempt
    ...
    FOR EACH ROW EXECUTE FUNCTION guard_production_release_target_recovery_attempt();
