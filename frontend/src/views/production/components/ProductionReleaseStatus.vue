...
async function load(next = 1, append = false) { const requestedScope = scope.value; const documents = [...props.documents]; if (!requestedScope) { releases.value = []; return }; loading.value = true; error.value = ''; await loadCoordinator.run(async () => releaseHistoryPage(await Promise.all(documents.map(row => listProductionDocumentReleases(row.id, next, 20)))), { success: history => { if (requestedScope !== scope.value) return; releases.value = appendReleaseHistory(append ? releases.value : [], history.rows); page.value = next; hasMore.value = history.hasMore }, error: cause => { error.value = cause instanceof Error ? cause.message : t('production.releaseConsole.loadFailed') }, settled: () => { loading.value = false } }) }
...
watch(scope, () => { mutationCoordinator.invalidate(); loadCoordinator.invalidate(); releases.value = []; page.value = 1; hasMore.value = false; load() }, { immediate: true })
