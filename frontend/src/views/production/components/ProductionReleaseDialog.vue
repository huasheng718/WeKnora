...
watch(() => [props.modelValue, props.documentId, props.versionId] as const, ([open]) => { mutationCoordinator.invalidate(); preflightCoordinator.invalidate(); loading.value = false; error.value = ''; if (open) { preflight.value = null; preflightTargets.value = []; selectedKnowledgeBaseIds.value = []; confirmed.value = false; releaseCommand = null; loadPreflight() } })
