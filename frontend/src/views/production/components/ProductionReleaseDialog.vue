...
async function confirmRelease() { if (!canConfirm.value) return; const signature = JSON.stringify([props.versionId, [...selectedKnowledgeBaseIds.value].sort()]); releaseCommand = releaseCommand?.signature === signature ? releaseCommand : { signature, command: createProductionCommand({ version_id: props.versionId, target_knowledge_base_ids: [...selectedKnowledgeBaseIds.value].sort() }) }; const mutation = mutationCoordinator.start(props.documentId); submitting.value = true; try { const response = await createProductionRelease(props.documentId, releaseCommand.command); if (!response.success || !response.data) throw new Error(response.message); if (mutationCoordinator.isCurrent(mutation, props.documentId)) { releaseCommand = null; emit('created', response.data); visible.value = false } } catch (e) { if (mutationCoordinator.isCurrent(mutation, props.documentId)) error.value = e instanceof Error ? e.message : t('production.releaseConsole.commandFailed') } finally { if (mutationCoordinator.isCurrent(mutation, props.documentId)) submitting.value = false } }
