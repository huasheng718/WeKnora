export default {
  ...
  production: {
    ...
    documentTypes: {
      ...
      fields: { name: "类型名称", origin: "来源", code: "类型 code", templateKey: "模板键", schemaVersion: "Schema 版本", status: "状态", actions: "操作", blockSchema: "内容块结构", sourceRequirements: "来源要求", skillBindings: "技能绑定", workflowPlan: "工作流计划", qualityRules: "质量规则", reviewPolicy: "审核策略", publicationPolicy: "发布策略" },
      origin: { builtin: "内置", custom: "自定义" },
      summaries: { sections: "共 {count} 个必需章节：{names}", sources: "至少 {minimum} 条证据；来源：{kinds}；证据章节：{evidence}", review: "审核步骤：{steps}", publication: "目标：{target}；已批准审核：{approval}" },
    ...
    documentWorkbench: {
      ...
      annotationStatus: { open: "待处理", resolved: "已解决", dismissed: "已忽略" },
      runStatus: { queued: "排队中", running: "运行中", waiting_approval: "等待审批", completed: "已完成", failed: "失败", cancelled: "已取消" },
      toolApproval: { not_required: "无需审批", pending: "待审批", approved: "已批准", rejected: "已拒绝" },
      providerTypes: { skill: "技能", mcp: "MCP", datasource: "数据源" },
      evidenceTypes: { text: "文本", json: "JSON", file: "文件", tool_result: "工具结果" },
    },
